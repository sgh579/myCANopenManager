...
if(!findTheFrame ||  m_readData.size()<6+DLC)
{
    //qDebug()<<"not found this time"<<endl;
    return;
}
ID=m_readData.at(frameHeadLocation+2);
DLC=m_readData.at(frameHeadLocation+3);
for(i=0;i<DLC;i++)
CANDataMessange[i]=m_readData.at(frameHeadLocation+4+i);
m_readData.remove(frameHeadLocation,6+DLC);//从字节队列中删除这一帧can转串口报文

//qDebug()<<"ID:"<<ID<<endl;
//qDebug()<<"DLC:"<<DLC<<endl;
//qDebug()<<"HEX:"<<CANDataMessange.toHex()<<endl;
QString str=CANDataMessange.toHex();
